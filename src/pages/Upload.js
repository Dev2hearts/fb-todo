...
const UploadJson = () => {
    ...
    const handleChangeFile = event => {
        ...
        setSelectFile(file);
        // 이미지 미리보기에서 보여준다.
        // 1. 복잡하게 처리(코딩이 길어지면)를 해야 하는 경우라면 함수를 만들고
    ...
    return (
        <>
            ...
            {/* 미리보기 */}
            {/* 자바스크립트 변수 및 기타 코딩을 배치할때는 {내용} 형식유지 */}
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div>
                    ...
                    {/* textarea 태그에는 절대로 타이핑시 Enter 키로 정리하지않는다. */}
                    <textarea
                        className="border"
                        id="jj"
                        value={jsonData}
...
const UploadMulti = () => {
    ...
    // handle 코딩 (이벤트 핸들링 함수)
    const handleChangeFile = event => {
        // 여러개의 파일을 선택하므로 배열로 받아온다.
        const files = Array.from(event.target.files);
        // state 변수에 선택된 파일이 여러개 이므로 files 배열을 저장한다.
        setSelectFile([...selectFile,...files]);

        // 화면에 출력할 이미지의 URL 문자열을 만들어야 한다.
        const imgPaths = files.map((item) => URL.createObjectURL(item));
        setPreviewImage([...imgPaths,...previewImage]);
    };
    const handleChangeJsonData = event => {
        setJsonData(event.target.value.trim());
    };
    const handleSubmit = async event => {
        ...
        selectFile.forEach( (item,idx) => {
            formData.append(`profil${idx}`, item);
        });
        const data = new Blob([jsonData], { type: "application/json" });
        formData.append("data", data);
